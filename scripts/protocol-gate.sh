#!/usr/bin/env bash
# protocol-gate.sh — this seat does not publish a wire protocol version that no
# published engine speaks (bl-52b5; yog bl-bca2 is the other direction).
#
...
# foot (`thrall`) and the phone each VENDOR a copy of the constant, and the
...
#      request until this repository's `main` — and thrall's, and
#      yog-android's — carries the new number. That is yog bl-bca2.
#   2. THIS repository publishing a bump ahead of the engine. thrall took that
...
#      install, composing with nothing. The seat's road is the same one.
#
...
# gate 1 waits for.
#
# STRICTLY GREATER, NOT DIFFERENT. A seat BEHIND the published engine is
...
# this repository's on `main` — and hands this script the paths.
#
# THE NUMBER IS A FILE, NOT A DECLARATION (bl-55b1, yog bl-3e57). Both reads
