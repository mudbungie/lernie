...
- Workflow advance (§1 #4, §6, "No resident interpreter") is scoped to workflow-event boundaries: a fresh `lernie advance` subprocess reads the repo and runs the next workflow action the state machine has reached, never picking up mid-conversation by reading `response.json`.
