//! BEP 44's mutable item — the shape yog's `docs/REMOTE.md` §13.2's presence
//! and inbox are written in: an ed25519 public key, an optional salt, a
//! sequence number and
...
/// An ed25519 rendezvous keypair — minted out of channel (yog's `docs/REMOTE.md` §1.4), held
/// by the end that publishes under it.
pub struct Keypair(Ed25519KeyPair);

impl Keypair {
    /// A fresh keypair from the system's randomness.
