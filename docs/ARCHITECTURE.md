...
Phase 2 lands the `dispatch` built-in tool — `lernie tool dispatch`, input `{role, goal}` — that realizes the §2.5 dispatch primitive on the model-facing side: it reads conversation context from the `LERNIE_CONV_REPO` / `LERNIE_CONV_BRANCH` env vars (§3.3), spawns the subagent through Phase 1's `lernie dispatch <role>` CLI (§3.4), and returns `{status: in_progress, handle}` synchronously. The subagent's own step loop and `await(handle)` are subsequent phases.
