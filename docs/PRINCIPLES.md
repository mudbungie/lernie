...
A workspace is one repo, agents are branches plus worktrees, configs are commits on descriptively-named branches, steps are commits, returns are messages, and the only merge is compaction (§2.6). Using git as the substrate gets history, branching, rollback, and replay for free — and fork *is* the freeze: an agent's config is the immutable commit it forked from (§2.2).

## One writer per branch
There is no `main`. A config branch advances only by user config edits; an agent branch advances only by its executor (§2.3). Every commit is provenanced to its lineage's single writer, and the only thing that ever merges into a branch is its own compactor's work, landed by its own executor (§2.6).
...
Termination *is* the return. When an agent finishes — or is stopped, or exhausts its budget — the **executor** deposits its result message into the parent's inbox (§2.6); the model never calls a return tool, because there is no return tool to call. This is the load-bearing automation, not a convenience. A harness that makes the child's result the *return value* of a tool (Claude Code's Agent tool, pi's `subagent()` extension) must instruct the model to close out explicitly, and then pays twice: with **truncation caps** — the result must fit the parent's context, so it is length-limited — and with **stalled parents** when the model simply neglects the close-out tool. lernie's result rides its own channel plus a work-product diff (§2.6), so the terminal response can just *be* the terminal response, and no model instruction is needed for it to arrive. The corollary the implementer must hold: the deposit is executor-side, outside the transcript — never a synthesized `message` tool call, which would forge a `tool_use` block the model never emitted and un-terminate the step (§2.3). `message` stays the tool an agent calls when it *chooses* to send.
