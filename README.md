...
cargo install brazen --version =0.0.5 --locked   # the provider adapter, always needed
...
cargo install brazen --version =0.0.5 --locked   # the pinned provider adapter
...
```
cargo install brazen --version =0.0.5 --locked
```
...
own version, `lernie <version> (brazen 0.0.5)`. Its one home is the
...
(`brazen = "=0.0.5"`) for the canonical *types* only — the data plane
...
```
bz version "0.0.4" does not match the linked brazen crate "0.0.5"
```
