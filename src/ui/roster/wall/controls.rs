//! **The acts on one aimed wall** — the eight controls that hang under the
//! aimed row, in the order DESIGN §4.20 fixes: its rank, the enrollment, the
//! four reads of its configuration, the fleet, and the unmaking last.
//!
//! Split from [`super`] at the design-time budget on the seam that module's
//! own doc draws: that file is *one workspace's row* — the line it wears and
//! whether this seat can address it at all — and this is *what an operator
//! can do to it once it is aimed at*. The first changes when a row grows a
//! fact; this one when a wall-level op lands a control.

use super::{PIN, UNPIN};
use crate::reply::roster::WsRow;
use crate::ui::Model;

/// **Paint the band and take its clicks**, in the order DESIGN §4.20 fixes:
/// the rank, the enrollment, the four reads of its configuration, the fleet,
/// and the unmaking last.
pub(super) fn render(ui: &mut egui::Ui, model: &mut Model, row: &WsRow) {
