#!/usr/bin/env bash
# lernie leak scan — the disclosure half of the gate. The rest of the gate asks
# whether the tree is well-formed (fmt, clippy, the 300-line cap, the ast-grep
# rules, cargo-deny, coverage); nothing asked whether it discloses something. A
# seat is the operator's own face on the harness: it holds this box's leaf and
# key, one entry per workspace held elsewhere, the address of every engine it
# dials, and — once the window lands — the rendered text of every conversation
# the operator has had. So the material it could leak is exactly the material
# it handles: key material, host addresses and topology, home paths, and
# transcript prose.
#
# PORTED FROM YOG — scanner, table, fixtures and both wirings. The copy is kept
# close to byte-identical on purpose: two copies of a disclosure gate drift
# within a week, and a short `diff` against upstream is the only cheap defence.
# Every deliberate divergence is commented where it sits. Ball ids in the
# commentary are YOG's and resolve in yog's store, not here.
#
...
# the same table judge lernie's index and the balls TASK STORE — a different
# git repo entirely (`<state>/balls/clones/<enc>/tasks`, holding `tasks/*.md`),
...
# rules for them would drift from this one inside a week. Its caller there is
# `scripts/lernie-leak-gate`, the balls plugin, before the store is pushed.
#
...
# ref, where a hit's remedy (a history rewrite) belongs anyway. lernie HAS a
# remote from its founding, and its store publishes to that same remote on the
# `balls/tasks` branch — so the question is live here and the check that would
# answer it is simply not written. Until it is, lernie has PREVENTION only:
# local, and bypassable by whoever runs it. That is a residual with a ball, not
...
# refs, pull-request and release text, CI logs, build artifacts and already-
# published crate versions are all outside it, and no hook can reach them; a
# gate that implied otherwise would be worse than one that says so. They are a
# RELEASE CHECKLIST instead — run by a person, once per publication, because
# every item on it is a one-time judgement whose remedy is destructive (a
# history rewrite, a yank, a rotation). yog's `CLAUDE.md` carries that list in
# full and lernie inherits it unchanged; the first `cargo publish` is the
# moment every one of those becomes reachable by somebody else.
#
...
# correctness rather than a style (bl-7ad6, reasoned in full at the head of
...
# forbidden path reported by nobody, at a rate that rises with load.
scan_paths() {
  local p
  for p in "$@"; do
