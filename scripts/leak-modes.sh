#!/usr/bin/env bash
# leak-modes.sh — the two SCOPES the scanner is asked in, split from the
# mechanism at the 300-line cap. Sourced by `leak-scan.sh`, never executed: it
# defines no CLI verb and reads `$FIXTURES` and every `scan_*` primitive from
# the file that sources it.
#
# Ported from yog unchanged: the file is pure mechanism and has
# nothing project-shaped in it. Cited ball ids are yog's.
#
