...
lernie additionally defines **message** as a structural term (`docs/ARCHITECTURE.md` §2.1, §2.11): content addressed to an *existing* conversation by any sender — the user or another conversation — deposited into the recipient's **inbox** (a conv-repo-root queue directory, namespaced by conversation id) and delivered by the recipient's **executor** (the single process driving the branch's step loop, §2.11 executor lock) as a committed worktree file at a step boundary. Distinct from the wire-level *user message* / *assistant message* senses above: in lernie prose, wire messages are always written qualified, and unqualified "message" means the §2.11 primitive.
