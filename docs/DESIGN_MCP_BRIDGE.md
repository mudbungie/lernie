...
**Ruling: deployment-owned bridge, no lernie product code.** The existing
...
**In-repo external binary (first-party `lernie-tool-mcp` shipped by lernie):
...
a first-party bridge would make the lernie repo ship, version, and gate code
...
*deployment* installs into the `lernie-tool-<name>` slot, exactly as the
fleet demo installs its Slack tools today. lernie's severability test passes
perfectly: deleting MCP support deletes deployment files (the bridge binary,
its config, the pinned triples), not a line of lernie code — because no line
of lernie code exists. The division of labor is brazen's, verbatim from
PRINCIPLES: "the binaries own wire protocols, vendor quirks, and credential
handling — lernie never sees a credential."
...
operator time, into an ordinary lernie tool triple in the data-root pools. A
**pinned tool** is the result: indistinguishable from any hand-authored
external tool. (Definition lives here; used nowhere in lernie code.)
...
lernie runs one tool subprocess per tool invocation; an MCP server wants to
...
- **Regenerability.** "Any process can die at any time without losing state.
  ...
  reconnect logic for a component lernie cannot even see.
...
lernie. It is not built now, because no deployment has charged the cost.
...
still not a lernie change. The seam does not move; that is the point of the
seam. One transport until one deployment proves it, per the ball's refusal.

## 5. Credentials: the bridge's, never lernie's
...
- **Ownership.** Server commands, arguments, and credential material
  ...
  rides lernie config, lernie env, or the tool's input schema.
...
`~/ops/fleet` ships `lernie-tool-slack_read` / `lernie-tool-slack_post` as
...
2. `bridge pin` writes the triples into the deployment's data root; `lernie
   config` (or a fresh `lernie new`) snapshots them.
...
4. **The pass criterion is a diff:** `git -C ~/dev/lernie diff` is empty.
   The whole exercise lands in deployment space.

## 8. Refusals (restated from the ball, now with mechanisms)

- **No MCP in lernie core** — §1; the TCB test and the brazen precedent.
