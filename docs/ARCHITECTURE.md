...
| **Executor** | The process currently driving a branch's step loop — `lernie prompt`, `lernie dispatch`, or a `lernie advance` re-entry (§6); the process `lernie stop` discovers by its executor-lock fd (§2.9, §2.11). | At most one per branch, guaranteed by the executor lock (§2.11). Distinct from the *tool executor* (§3.2), a role inside it. |
...
> **Shipped-state note.** The v0.3 merge-back protocol this section retires is **deleted** (bl-4ce8): `src/prompt/merge/` (rebase-then-align-then-merge), the `merge=ours` `.gitattributes` and `merge.ours.driver` scaffold, `MERGE_OURS_PATHS`, and `tests/merge_ours.rs` are gone. The result-message return is shipped as its replacement: `inbox::deposit_result` (the reusable, total deposit — `epitaph:`/`terminal_ref:` frontmatter, body = terminal response iff the agent spoke; §2.11 shape) and `inbox::deposit_child_result` (its parent-derivation wrapper, a no-op for a root), wired at **every** terminal event the root's `run_exchange` loop reaches today — normal completion (`final-response`), budget exhaustion (`budget-exhausted`, §6), and stop (`stopped`, §2.9, bl-9f53). The stop wiring is the executor-side SIGTERM handler (`src/prompt/dispatch/stop_signal.rs`): a lone async-signal-safe flag store, read at the step loop's check points (before each model call, and immediately after — a stop mid-call surfaces as an `AdapterHalfStream` the loop then treats as a stop, not a failure), breaking to the on-the-way-out deposit (`src/prompt/dispatch/terminal.rs`). The **work-product transfer** (`src/prompt/dispatch/transfer.rs`) is live at delivery: a delivered message carrying `terminal_ref:` applies the fork-point→terminal diff, filtered to work products, as one commit before the delivery commit, and a diff that fails to apply is **declined loudly** at `refs/lernie/conflicted/<agent-id>`. What is *structurally wired but dormant*: the **deposit** call sites in `run_exchange` land nothing today because a root has no parent inbox and the shipped subagent path (`worker.rs`) stops at the dispatch commit — children do not yet run a step loop, so no child reaches a terminal event. The stop *handling* around that no-op deposit — catch SIGTERM, do not die, deposit-attempt, exit cleanly with the missing-`end` signature on `response.json` untouched — is itself live and proven end-to-end against a real signal (`tests/stop_cli.rs`: `lernie stop` on a real root harness; the executor exits 0 rather than dying). `deposit_result` is total and reusable so the §8 sweep (bl-d148, behind the operator verb `lernie scan`, §2.11) can deposit `died` epitaphs on a crashed child's behalf, and the child step loop lands the live child returns. The one surviving merge is compaction (`src/prompt/compactor`), a plain `--no-ff` merge with no rebase or alignment.
...
Stops are aggressive but agent-scoped. A stop targets **one agent** — the user-driven case is the CLI subcommand `lernie stop <workspace> <agent>` per §3.4, idempotent against an already-terminal branch; the timeout/budget case is the same act issued by the harness (§6). It does **not** reach across the agent boundary into that agent's children by default: parent and child are separate agents, not a process hierarchy, so stopping a parent leaves a running child alive to finish and revive the parent on its return deposit (§2.5, §2.11). The agent→agent cascade is opt-in — `lernie stop --stop-children` (below) — never a kernel-group side effect. When a stop is issued:
...
> **Shipped-state note.** The deposit half of the channel is shipped (bl-cb44): the executor lock (`flock` on the agent's `inbox/<agent-id>/` fd, `src/prompt/inbox/lock.rs`, acquired at the start of the shipped `lernie prompt` step loop and released on exit), the create-only deposit (`src/prompt/inbox/deposit.rs`), the `lernie message <workspace> <agent> <content>` verb, and the `message` built-in tool (`src/prompt/tool/builtin/message/`, sender from `LERNIE_CONV_BRANCH`). The **step-boundary drain is shipped** (bl-1129, `src/prompt/dispatch/drain.rs`): the executor's step loop, before it captures each step's read state, commits any renamed-but-uncommitted stray in `messages/`, then moves each pending `inbox/<agent-id>/<sender>-<NNN>.md` into the worktree as `messages/<counterNNN>-<sender>.md` (a literal `rename(2)`, drawn on the branch's transcript counter inside the executor lock) and commits the move — the delivery commit — in a deterministic `(mtime, filename)` order. The initial user message now rides this same path: the root executor deposits it into the agent's own inbox and the step-1 drain delivers it (the front-door unification the §2.4 reprompt rule implies), so the §2.3 direct-commit stand-in is gone. The **result-message return path** is now shipped too (bl-4ce8, bl-9f53): the result-message deposit (`inbox::deposit_result` / `deposit_child_result`), the epitaph deposits wired at the root's terminal events (final-response, budget-exhausted, and stop — the executor-side SIGTERM handler of §2.9 step 3, bl-9f53), and the delivery-time **work-product transfer** with its declined-transfer mark are all built (§2.6 Shipped-state note) — the epitaph deposits are structurally wired but dormant until a child runs a step loop, since a root has no parent inbox; the stop handler's catch-deposit-and-exit-clean behavior around that no-op is itself live and real-signal-tested (`tests/stop_cli.rs`). One spawn remains stubbed on the channel: every launch seam — the verb's post-deposit probe, the exit protocol's self-directed launch (below, bl-5846), and the `lernie scan` flush — is live decision logic calling one injected launcher, but the driver it launches is `lernie advance` (§6), which does not yet exist, so the production spawn is a documented no-op pending that verb; a deposit into a quiescent agent waits until the next driver runs against the branch, whose drain then delivers it. This section is otherwise the target contract.
...
**Undelivered is derived.** A deposit can land in the gap between an executor's final drain and its lock release; the exit protocol (below) closes this race structurally on the graceful path — the exiting executor launches a driver *after* releasing, and that driver finds the deposit — and the writer's post-deposit probe covers the case where the executor is already gone. Both are actions against observed state, so the classification stays on disk rather than in anyone's memory: a message file still under `inbox/<agent-id>/` whose agent has no live executor is *undelivered* — a derived classification (inbox listing + the lock probe), never a flag. Under normal operation it is transient — the next driver, launched by any deposit or started by hand, delivers it. The sender observes its outcome the way anything is observed here: its file left the inbox in a delivery commit, or it didn't. Undelivered count is a §8 health metric alongside silent deaths.

**The exit protocol: the self-directed launch.** The channel above carries all of normal operation by itself: `lernie message` deposits, probes, and launches; the executor drains at every step boundary; and deposit-before-probe ordering means a queued item exists before anyone checks — the healthy path needs zero scanning and zero inbox-watching. What it leaves open is the graceful-exit crack: a deposit landing *after* the executor's final drain but *before* its lock release meets a lock-held writer probe (nothing launched) **and** an executor that will never look again. The close is unconditional, not a conditional post-release check: **one terminal sequence, no agent kinds** — reach a terminal event → deposit the result message into the parent's inbox (§2.3 step 5, §2.6; a structural no-op for a parentless agent, which has no parent inbox) → release own lock → spawn a driver at own agent, fire-and-forget → exit. The only conditional in the sequence is "does a parent inbox exist," derived from the id (§2.3).
...
1. **The no-op driver path.** A driver that acquires and finds nothing to deliver on a terminal branch exits silently — no step, no epitaph, no further launch. This structurally terminates the recursion: found-mail → step to a new terminal → exit-launch again (the normal lifecycle); found-nothing → nothing relaunches. The same path is what makes racy launches free (needed anyway): N deposits may each launch a driver, one wins the acquire, the rest exit as clean no-ops.
2. **Exit-launch by epitaph value** (§2.6: code branches on the epitaph's value, never on message shape): *final response* → launch; *stopped* → never — a relaunch would resurrect the branch the operator just killed; pre-stop mail waits for the next explicit touch, and future deposits still revive the branch per §2.9; *budget-exhausted* → never — drain → exhaust → deposit is an epitaph-spam cycle against a hard ceiling.

The cost is one no-op spawn per normal termination — termination frequency, not step frequency — accepted at current scale and optimized only if measurement shows it. The spawn is detached — its own process group (`setsid`), stdio to a log or null — so the §2.9 stop cascade against the exiting executor never reaches the driver it launched, and the exiting process does not linger to watch it (it has no authority to act on the outcome anyway). A spawn failure falls into the accepted crash class below. inotify is a correctness mechanism nowhere in this: holding an inbox fd confers no notifications and inotify queues overflow; it may serve as a mid-loop latency optimization only (the same advisory status as §3.1's frontend watching). The authoritative read is the drain's readdir under the lock.

**Crashes are a failure class, not a scan trigger.** Uncatchable death (SIGKILL, OOM, panic, reboot) strands what it strands: a dead child's undeposited result parks its parent until the next touch, and messages deposited while a doomed executor lived wait as undelivered. Everything is on disk, so nothing is lost, only late — and the next touch heals it. That touch is a user reprompt anywhere (the deposit starts a driver, above), or the operator verb **`lernie scan`**: one workspace-wide pass with two derived actions. First, the **silent-death sweep** (§8): for each hard-crashed child in the candidate set — no live executor, died mid-work or never deposited — it deposits the `died`-epitaph result message on the child's behalf (§2.6, §2.9). Second, the **inbox flush**: it lists `inbox/*/*`, and every agent with pending files and a free lock gets a driver launched for it — *launched, never drained*: the scanner moves no files and commits nothing; only an agent's own lock-holding executor delivers (Writer/driver totality, above), and an agent whose lock is held is left alone — its executor drains at its next boundary. The sweep's own deposits are picked up by the flush that follows in the same pass. `lernie scan` is an **operator tool** — run by hand, or by cron if an operator wants a heartbeat (config, not schema) — and is never wired into any hot path or default schedule: the events it compensates for happen at crash rate, not step rate, and wiring it into driver startup would both pay O(branches) per spawn and hand the worst failure modes an accelerant — an automatic sweep relaunching the runaway agent an operator is trying to kill. A fully idle workspace therefore stays unswept until its next touch, and that is accepted rather than patched: with nothing running, nobody is blocked on the outcome. The stalled-parent story needs no new verb: reprompting a parked parent starts a driver whose drain delivers whatever is pending, and a hand-run `lernie scan` deposits a dead child's epitaph and launches the drivers to deliver it.

> **Shipped-state note (bl-d148, bl-5846).** The scan is shipped (`src/prompt/inbox/scan/`) behind the operator verb **`lernie scan <workspace>`** — and *only* there: no driver startup runs it (`lernie prompt` / `lernie dispatch` touch only their own branch). The sweep's candidate derivation is built in full: the executor-lock probe, the died-mid-work classification (the framing-tail `response.json` read via the §4.4 `segment::classify`, framing-only), the sender-namespaced never-deposited derivation (parent inbox *and* transcript), and the `died`-epitaph deposit-as-scribe; idempotence holds because the never-deposited test sees the sweep's own prior deposit. The **exit protocol is shipped** at the root step loop's terminal seam (`dispatch::run_exchange` / `dispatch::terminal`): deposit (§2.6 note) → explicit lock release → epitaph-valued launch decision (final response launches; stopped and budget-exhausted never; an executor that errors out deposits nothing and launches nothing — the accepted crash class). The launched driver's own-branch entry — acquire-or-exit, deliver pending mail, or exit silently on an empty inbox (pin 1) — is `dispatch::driver::drive`, the seam `lernie advance` (§6) will run. Every launch seam — the writer's post-deposit probe, the flush, and the exit launch — calls the one injected launcher, whose production target is `lernie advance`; that verb does not yet exist, so the spawn is a documented no-op pending it, and the detached-spawn contract above (own pgid, stdio to log/null) lands with it. Two divergences from this section's wording, both honoring shipped reality: (1) agent branches are **bare** (`<conv-id>` / `<parent>-<sub-id>`), not `agents/*` (§8), so the enumeration is *every branch except `main`* — the one seam to change when a prefix lands; (2) a child does not yet run a step loop (§2.5 worker path stops at the dispatch commit), so a real died-child cannot arise from a run — the derivation is exercised against constructed on-disk states, which is also its honest unit.
