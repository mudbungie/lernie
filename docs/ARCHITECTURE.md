...
> **Shipped-state note.** The shipped harness still carries the pre-transcript shape this amendment retires: the step loop folds assistant events into an in-memory accumulator and assembles `tool_result` blocks from `tools/<tool-id>/output.json` (see README). This section is the target contract; the writer's mechanics are specified below (*The transcript writer*) and the implementation is tracked as bl-4798 (the writer) and bl-26cb (re-pointing the assembler, deleting the accumulator and the `output.json` read path).
