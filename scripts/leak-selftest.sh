# lernie leak scan — the regression half, and the reason the gate cannot rot.
# Ported from yog with the scanner; one case is dropped, and the
# comment where it stood says which and why.
#
...
#
# A `grep -q` HERE READS FROM A HERESTRING, NEVER FROM A PIPE, and every
# tracked BASH script under `scripts/` is held to that by the check at the
# foot of `self_test` (bl-7ad6). A `grep -q` on the receiving end of a pipe is a race,
# not a style: it exits the instant it matches and closes the read end, the
# writer is killed by SIGPIPE part-way through its own write, and `set -o
# pipefail` then takes the pipeline's status from that DEAD WRITER rather than
# from the reader that answered — so the pipeline reports FAILURE exactly when
# the pattern MATCHED. That is what reported a live rule dead in one `make
# check` and passed on the next two; measured on a loaded box it answered
# falsely 36 times in 16,000 pipelines, and 0 times in 16,000 herestrings.
...
# inherited it, which is how the defect reached here — and a herestring has no
# second process to die under either setting.
...
fixture_lines() {
  ...
  # says nothing about why — so "this rule matched nothing" and "this file
  # could not be read as text here" would arrive as the same sentence, and only
  # the second is a fault of the box rather than of the gate. A fixture is
  # tracked text; if it does not read as text in this locale, the run has no
  # verdict to give about the rule.
...
self_test() {
  ...
  # Upstream has one more case here: a DECLARED DERIVATION that must NOT be
  # flagged, proving the `BINARY_ALLOWED` arm still lets its one entry through.
  # lernie's allowlist is empty (`^$` — see leak-rules.sh), so there is no such
  # file to name and a case here would have to invent one. The day a tracked
  # binary is declared, it arrives with its regeneration test AND with the line
  # that belongs here; that is the same "each lands with the surface it
  # governs" rule the ast-grep confinement rules were held to.
  ...
  # file says why (bl-7ad6). This is the two-direction discipline turned on the
  # harness itself: the fixtures prove a rule still bites, and this proves that
  ...
  # it runs unattended on a seat box, and dash has neither the option that
  # makes the shape wrong nor the herestring that fixes it. A file with no
  # `#!` is a sourced bash fragment and IS in scope: this one is, and it is
  # where the defect lived. The fixtures are data, not code.
  #
