...
> **Ladder transition (in progress).** This table is terminology-ladder v2: the structural primitive is the **agent** (not "conversation"), its container is the **workspace** (not "conversation repo"), and the versioned configuration it forks from is a **config**. §§2.2–2.11, §3, §8, and `docs/PRINCIPLES.md` still carry the pre-ladder vocabulary ("conversation," "conversation repo," "subagent," and the `main`-rooted root-history model); those sections are slated for the workspace/substrate rewrite (tracked separately). Until then, read "conversation" as *agent*, "conversation repo" as *workspace*, and "subagent" as *child agent*. Where a term's full mechanics land in a later section, this table defines the term and defers the mechanics.
...
| **Soul** | The system prompt handed to an agent at dispatch, drawn from the workspace's `souls/<role>.md` and overwritten on the branch's dispatch commit. | One per agent; composed into every model call on the branch as the system message. |
...
**Retired structural terms** (v0.1–v0.3 vocabulary, superseded by the ladder above; still present as pre-ladder prose in §§2.2–2.11, §3, §8, and `docs/PRINCIPLES.md`, pending the workspace/substrate rewrite):
...
A **conversation repo** is the on-disk materialization of one root conversation plus all its subagent descendants. It lives at `<data-root>/conversations/<root-id>/`.

**Harness root, split by XDG lifetime.** The harness root is installation-global state outside any one conversation repo, split along XDG lifetimes into two homes:

- **config root** — `$XDG_CONFIG_HOME/lernie` (default `~/.config/lernie`). Hand-edited declarations: the global `models.yaml` (§4.2) and the `workflows/` templates.
- **data root** — `$XDG_DATA_HOME/lernie` (default `~/.local/share/lernie`). Machine-populated state the harness copies from at bootstrap: the `conversations/` tree and the `agents/`, `skills/`, and `tools/` pools.
...
**Step records are not context.** A **step record** is the per-step on-disk directory at `<conv-repo>/steps/<conv-id>/<NNN>/`: `meta.json`, `request.json`, `response.json`, and any `tools/<tool-id>/` subdirectories the step emitted (full layout in §2.3). The whole `steps/` tree at the conversation-repo root holds these diagnostic / audit artifacts. It sits outside every worktree by construction, which means context assembly (§3.5, §5) is *physically incapable* of including it. The structural placement enforces the rule the worktree-as-context invariant (§5.1) implies: an agent's context — running, retried, or replayed — is assembled from the branch's read-state commit and nothing else. The model-facing history itself is committed worktree data, the **transcript** (§2.1, §2.3): prior assistant output and tool results are transcript entries, so no runtime code path reads step-record *content* at all. `response.json`'s event *framing* — the terminal `end`, `Finish`/`Error` kind, `Usage` — is read for classification and metering (§2.3 Diagnostic-only contract); everything else in this tree is write-only diagnostic. See the diagnostic-only contract in §2.3 for the full split.

**Frozen-copy bootstrap.** Creating a repo is `cp -r <data-root>/agents/<profile>/* <data-root>/conversations/<root-id>/` plus `git init` plus the initial commit. All control files are frozen snapshots at that point. Subsequent changes to the global agent profile, to the `workflows/` templates under the config root, or to `<data-root>/skills/` or `<data-root>/tools/` do not propagate into existing repos. Reproducibility and portability win over live update. (Endpoints and auth are not lernie's to copy — since v0.6 they live entirely in brazen's own config and credstore (§4.4), never a harness file; the per-repo `providers.yaml` carries only the role → (provider row name, model id) pointer, and the global `models.yaml` under the config root holds model capabilities and context windows (§4.2). brazen resolves endpoint and auth from the row name at call time (§4.1).)
...
3. **Work.** The agent runs its loop. At each step boundary the executor drains the conversation's inbox — pending messages land as a delivery commit before the model call (§2.11). When the model call completes, the executor commits its assistant output as a transcript entry (below); every tool call commits its result as a transcript entry when it resolves, and worktree-modifying tool calls (e.g. `bash` editing files) carry those modifications in the same serialized commit discipline (§3.3). Each step's diagnostic record (§2.3 Step on-disk layout) lands at `<conv-repo>/steps/<this-conv-id>/NNN/`, outside the worktree. A step that emits a subagent-targeting tool call spawns a sub-branch off the branch tip.
...
- `messages/NNN-assistant.json` — one step's assistant output: the canonical content blocks (text, thinking, `tool_use`) exactly as the step's authoritative attempt segment emitted them (§4.4), written by the executor from the adapter stream it is already consuming — one pass, two sinks (the diagnostic `response.json` and the transcript), no read-back — and committed when the model call settles complete; a retried or paused model call commits only the authoritative content (§2.10, §4.4). Composes verbatim as an assistant-role wire message.
...
The assembler walks the sequence and groups consecutive same-side entries into alternating wire messages; because tool results commit immediately after their emitting step's assistant entry, the provider pairing rule — every `tool_use` matched by a `tool_result` in the immediately following user message (§2.5) — holds by construction. Path-derived framing is forced, not chosen: the wire itself requires role structure (a single flattened user block cannot carry `tool_use`/`tool_result` pairing, and replayed thinking blocks must re-enter under their original role with their original signatures for the request to be bit-identical), and the role's single home is the filename — frontmatter would be a second copy of a fact the path already states. The `.json` entries carry brazen's canonical vocabulary verbatim (§4.4), which is what makes replay bit-for-bit rather than a lossy re-rendering. (`assistant` and `tool` are reserved origin tokens; senders are agent ids or `user`, §2.11.)
...
*Merge-back.* At terminal compaction the child's appended transcript entries and its branch-scoped `summary/` files are **deleted** — its history stays on the child branch's commits for provenance, and its distilled result travels through the ordinary channel (the terminal summary returned as the parent's `await(handle)` `tool_result`, §2.5, §2.7) — so the merge-back diff carries no `messages/**` or `summary/**` paths at all: the parent's post-fork appends and the child's can never collide, structurally rather than by pinning (§2.6).
...
These files are all branch-scoped: each conversation writes its own `goal.md` and `soul.md` on the dispatch commit (§2.3 step 2), its own `summary/NNN.md` as compactions happen (§2.7), and its own `messages/` transcript entries as steps resolve and deliveries land (§2.3, §2.11). Without the discipline, every subagent merge-back would carry the subagent's versions of these files into the parent's tree — which is precisely wrong, since the parent resumes with *its* goal, not the subagent's. With it, the subagent's versions stay on the subagent branch (visible in history for provenance, not reflected in the parent's post-merge state).

**Most of this discipline is now structural.** With the transcript in the worktree (§2.3), terminal compaction deletes the child branch's appended transcript entries and its branch-scoped `summary/` files before the merge — both superseded by the terminal summary the parent receives through the handle (§2.5, §2.7) — so the merge diff carries no `messages/**` or `summary/**` paths at all: nothing to conflict, nothing to pin. What the alignment step still earns is `goal.md`/`soul.md` (overwritten in place at the child's dispatch commit, so the child's diff *does* touch them) and restoring any *inherited* context file the child deleted as curation (§5.4) — a child's `rm` of an inherited transcript entry must not propagate to a parent that still wants it. The `.gitattributes` entries stay in place as the backstop for human-driven merges. Whether that residue — and merge-back itself — survives at all belongs to the workspace-substrate rewrite (§2.1 ladder note); it is resolved there, not here.
...
**Compaction is the sole sanctioned reorganization point — and it is a merge operation.** Between compactions, assembly is append-only (§5.5): the assembled context only grows at the tail, keeping the provider's cached prefix valid. The compaction merge — the compactor merging back into the dispatching branch — is where the context is rebuilt: superseded transcript entries deleted, a new summary landed, the manifest's category order reapplied at the next assembly (§5.2), and the cache prefix deliberately flushed. The merge commit *is* the rebuild point (§5.5); no other event reorganizes a running branch's context. This is also why the compaction merge is exempt from the merge=ours discipline (§2.6): that discipline protects a parent from a *child's context*, while the compaction merge exists precisely to rewrite the dispatching branch's own context files — they are its payload, not its contamination.
...
- **Tool executor.** Runs tool subprocesses on behalf of the harness; contract in §3.3. Per tool call: assembles stdin from the `tool_use.input` the model emitted; invokes the tool binary (`lernie tool <name>` in-process or `lernie-tool-<name>` external); captures stdout and stderr atomically (temp path + rename) into the diagnostic `<conv-repo>/steps/<conv-id>/<NNN>/tools/<tool-id>/output.json` (out of every worktree, §2.2, §2.3); derives the canonical `tool_result` block (stdout as content, stderr concatenated on non-zero exit, exit code as `is_error`) and commits it as a transcript entry `messages/NNN-tool.json` on the emitting branch (§2.3, §3.3), from which the next step's request payload composes. Cascades SIGTERM on cancel (§2.9) with a 5s deadline before SIGKILL (§3.3). Termination by a signal other than the harness's own SIGTERM (SIGSEGV, SIGABRT, etc.) is a harness-level fault per §2.10. Does not auto-dispatch on oversized tool output in v0.3 (§11).
...
**Skill.** A directory containing a `SKILL.md` with YAML frontmatter (`name`, `description`) plus markdown instructions, optionally with bundled scripts and reference files. Skill source directories live globally at `<data-root>/skills/<name>/` and are referenced from the conversation repo by two mechanisms:
...
**Tools-list assembly.** A role's enabled tools are declared in the per-repo `<conv-repo>/providers.yaml` `roles:` section under a `tools: [...]` field (see §4.3). Any on-disk triple (binary + schema + skill) is generally discoverable; role configs select from that pool. At request-assembly time the harness composes the declared names against the schemas committed in the branch's read-state tree (§2.10): each declared tool whose `descriptions/tools/<name>.json` schema is present becomes one entry in the canonical request's `tools: [...]` array, with that file sent verbatim as `input_schema`; a declared name with no committed schema is dropped (the intersection of declaration and availability), and a present-but-malformed schema is declined rather than dropped. **Shipped-state note:** the consumer (this assembly) shipped with bl-9e96, but the descriptions-always population (next paragraph) is not yet wired — no conversation-repo creation routine exists in production to write the initial `main`, so `descriptions/**` is empty and this assembly currently composes an empty `tools: [...]` array. Building the producer as that creation-routine step is tracked as bl-3092. A second consequence follows from producer-after-consumer ordering: an entry's `input_schema` (from `descriptions/tools/`) composes independently of and lands before its `description` (point 3, from `SKILL.md` frontmatter via `descriptions/skills/`), so an entry may transiently carry a schema with no top-level `description`.

**Descriptions-always population.** The `descriptions/**` tree the assembly above intersects against is written by a single step of the conversation-repo creation routine (§2.2), as part of the initial `main` commit: the harness copies, from the data-root pools, every available tool's JSON schema (`<data-root>/tools/<name>.json` → `descriptions/tools/<name>.json`, point 2) and every available skill's `SKILL.md` frontmatter (`<data-root>/skills/<name>/SKILL.md` → `descriptions/skills/<name>.md`, the Description-always mechanism above). It is **one mechanism over two artifact kinds, not two producers.** Committing onto `main` is what lets every branch inherit the descriptors via git (§2.3) and makes context assembly intersect against the branch's own read-state tree (§2.10, §5.1) instead of re-reading mutable data-root state — the same snapshot-not-symlink discipline as skill bodies above and as `providers.yaml` "frozen at creation" (§2.2). The data-root pools are the single source of truth for *what this install provides*; the committed `descriptions/**` snapshot is the single source of truth for *what this conversation is pinned to see* — distinct facts that must not drift together, so the copy is a snapshot, not a mirror (PRINCIPLES, single source of truth).
...
Each role's system prompt is read from `<conv-repo>/souls/<role>.md` by convention — there is no per-role path override, and no freeform path field to validate. At dispatch time the harness copies the appropriate soul to the new branch's `soul.md` (§2.3 step 2). `provider:` names a brazen row; endpoint and auth resolve at call time inside brazen (§4.1) — the per-repo file carries only the (row-name, model-id) pointer, cross-validated against `<config-root>/models.yaml` at load.
...
roles:
  ...
  compactor:
    ...
    # inherited worktree: forked off the tip, it carries that branch's
    # transcript, summaries, and work products (§2.3, §2.7). The
    # compactor's manifest sees only its own worktree, same as any
    # other role.
    budget_tokens: 50000
    overflow: truncate
```
