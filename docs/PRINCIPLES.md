...
Every piece of data is recorded in at most one place. If git already carries it — branch existence (`git branch --list`), commit history (`git log`), tree contents (`git ls-tree`), the fork point of a branch — we don't also write it into a JSON state file. If the filesystem already carries it — a file's presence, a directory's contents — we don't also record that in a sidecar manifest. Copy is the bug: two sources means a consistency protocol, and a consistency protocol means drift. When in doubt, derive don't mirror. This is why the harness tracks unmerged branches via `git branch --list ex/*` rather than a `branches.json`: the ref database is authoritative and free, and any parallel record of the same fact is dead weight at best and contradictory state at worst.
