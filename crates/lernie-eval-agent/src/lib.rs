//! `lernie-eval-agent` — the harness driver the evaluation runner
...
//! the task prompt on argv\[1\], `LERNIE_HOME` / `LERNIE_EXPERIMENT` /
//! `LERNIE_EVAL_REPORT` in the env, and the working directory shared
//! with the task's `setup` and `check`. This crate is that program for
//! the lernie harness itself. It is an integration, so it is an
//! external binary (`docs/PRINCIPLES.md`), **not** a `lernie` verb —
//! and it drives the harness exclusively through the front door, by
//! exec'ing `lernie` (resolved from `PATH`):
//!
//! 1. Seed the run's isolated `LERNIE_HOME` from the machine's lernie
...
//!    seed-if-absent, exactly like `lernie prime`.
//! 2. `lernie new` — create the run's workspace.
//! 3. **Apply the experiment** (the quiet end of the seam):
//!    `lernie config` with `$EDITOR` set to copy `LERNIE_EXPERIMENT`
...
//! 4. `lernie prompt` — one root agent, handed the task prompt grounded
//!    in the shared working directory (see [`grounded`]).
//! 5. Report the workspace path and agent id through
//!    `LERNIE_EVAL_REPORT` (two lines), which is what `lernie bundle`
...
pub struct Contract {
    ...
    pub prompt: String,
    /// The isolated harness root for this run (`LERNIE_HOME`).
    pub lernie_home: PathBuf,
    /// The experiment `workflow.yaml` to apply (`LERNIE_EXPERIMENT`).
...
impl Contract {
    ...
    pub fn assemble(
        ...
        Ok(Self {
            ...
            lernie_home: lernie_home.ok_or("LERNIE_HOME is not set")?,
            experiment: experiment.ok_or("LERNIE_EXPERIMENT is not set")?,
            report,
            workdir,
        })
    }
...
/// The machine's lernie config root — `$XDG_CONFIG_HOME/lernie`, else
/// `~/.config/lernie` (ARCH §2.2) — resolved from the given env values,
/// deliberately ignoring `LERNIE_HOME`: for the driver that names the
...
pub fn machine_config_root(
    ...
    if let Some(x) = xdg_config_home.filter(|x| !x.is_empty()) {
        return Some(PathBuf::from(x).join("lernie"));
    }
    home.filter(|h| !h.is_empty())
        .map(|h| PathBuf::from(h).join(".config").join("lernie"))
}

/// The prompt handed to `lernie prompt`: the task prompt grounded in
...
/// Run the whole driver against a `lernie` program: seed, `new`, apply
/// the experiment, `prompt`, report.
pub fn drive(lernie: &OsStr, machine_config: Option<&Path>, c: &Contract) -> io::Result<()> {
    if let Some(root) = machine_config {
        seed_home(root, &c.lernie_home)?;
    }
    let workspace = non_empty(product(lernie_command(lernie, c).arg("new"))?, "new")?;
    apply_experiment(lernie, c, &workspace)?;
    let agent_id = non_empty(
        product(
            lernie_command(lernie, c)
                .arg("prompt")
                .arg(&workspace)
...
/// machine has them. Seed-if-absent, like `lernie prime` (ARCH §2.2) —
...
/// end of the §9.3 seam): `lernie config` materializes the authoring
/// checkout and hands it to `$EDITOR` (as `exec $EDITOR "$1"` through
/// `sh`), so an "editor" of `cp -f "$LERNIE_EXPERIMENT"` lands the
...
fn apply_experiment(lernie: &OsStr, c: &Contract, workspace: &str) -> io::Result<()> {
    let mut cmd = lernie_command(lernie, c);
    cmd.arg("config")
        .arg(workspace)
        .env("EDITOR", "cp -f \"$LERNIE_EXPERIMENT\"");
    product(&mut cmd).map(drop)
}

/// A `lernie` invocation carrying the run's env: the isolated
/// `LERNIE_HOME`, the experiment path (for the `$EDITOR` hand-off),
...
fn lernie_command(lernie: &OsStr, c: &Contract) -> Command {
    let mut cmd = Command::new(lernie);
    cmd.current_dir(&c.workdir)
        .env("LERNIE_HOME", &c.lernie_home)
        .env("LERNIE_EXPERIMENT", &c.experiment);
    for var in [
        "GIT_DIR",
        "GIT_WORK_TREE",
        "GIT_INDEX_FILE",
...
fn non_empty(product: String, verb: &str) -> io::Result<String> {
    if product.is_empty() {
        Err(io::Error::other(format!(
            "lernie {verb} printed no product"
        )))
    } else {
        Ok(product)
    }
...
/// Run one `lernie` verb and return its stdout product (ARCH §3.4: one
