...
5. **Return.** A child deposits its **result message** into its parent's inbox (§2.6) — its terminal response plus its terminal ref, from which delivery applies the work-product transfer. A root agent's terminal response answers the user (§2.4, §7.1). Nothing merges anywhere; the branch simply stops advancing.
...
**Async work uses handles.** Long-running tools — including dispatch — return immediately with a handle (`{status: in_progress, handle: <id>}`) as their `tool_result`. The agent retrieves the outcome later via a separate `await(handle)` or `check(handle)` tool, whose return value is a `tool_result` on a later step. Parallelism is expressed by issuing several dispatches in one step and awaiting them in subsequent steps as results come in. The per-step "one result per tool_use" shape stays intact; the asynchrony rides on the handle. Dispatch is a tool like any other: this is how the symmetry between inline tool calls and child agents is preserved without inventing a second control path. The v0.4 `dispatch` built-in — `lernie tool dispatch`, input `{role, goal}` — realizes this contract: it spawns the child through `lernie dispatch <role>` (§3.4) and emits the handle as `<this-agent>-<child>` — the child's agent id, which is also its branch name and its address (§2.3, §2.11).

**`await` is waiting for a message.** A handle resolves through the inbox, not through a merge. The blocking form, `await(handle)`, parks until the named child has either deposited into this agent's inbox — its result message (§2.6) or any upward message — or died silently. Its `tool_result` is *status only*, derived on the spot: `{status: returned}` when a message from the child exists (in the inbox, or already delivered as a transcript entry — both derived from filenames, §2.3, §2.11); `{status: stopped}` or `{status: budget_exhausted}` for a child with no live executor and no deposit, derived from the §2.9 on-disk signatures (a closed `response.json` without a terminal `end`; the budget ref, §6) — the fallback for children that die without returning. The message *content* is never duplicated into the `tool_result`: it delivers at the next step boundary as the ordinary transcript entry it is (§2.11) — content has one channel and one home. The non-blocking form, `check(handle)`, is the same derivation returning immediately, with `{status: in_progress}` while the child holds its lock and has deposited nothing. (`{status: conflicted}` is retired with the merge protocol; its survivor is the declined work-product transfer, marked git-natively at delivery, §2.6.)

Tool calls targeting non-dispatch tools commit their records on the emitting branch as part of the step. Dispatch tool calls fork child branches off the commit where the dispatch landed; the `tool_result` the parent step observes is the handle. When the child returns, its result message delivers at a step boundary like any deposit (§2.6, §2.11); `await(handle)` is how the parent blocks for it, `check(handle)` how it polls.
...
In every other respect the compactor is an ordinary child: its handle answers `check`/`await` (§2.5) like any other, and the dispatching branch does not idle while it runs (§2.6).
...
A stopped agent deposits no result: a stopped child's parent derives `{status: stopped}` through `check`/`await` (§2.5), and a stopped root agent's exchange ends without a terminal response (§2.4). The branch remains as a ref for retention (§9.2), and a stop is not a locked door: the paths forward are the ordinary primitives — a message into the stopped agent's inbox starts a driver and resumes the same branch (§2.4, §2.11), or a fork off any of its commits starts a fresh agent (§2.3). There is no distinct "resume" operation — message-and-drive and fork-from-history subsume what resume would have done, and a third name would only obscure that.
...
- Tokens per step, per agent, per workspace — summed across *every* attempt segment of each step's `response.json`, not only the authoritative last segment; failed and superseded attempts are billed (§4.4, §6 budgets).
