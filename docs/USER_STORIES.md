...
- **status** **fulfilled.** `src/e2e/advance_cli.rs::message_launches_a_detached_advance_chain_that_batons_through_tools` drives real subprocesses and real `bz` through three model calls and a real `bash` tool call, proving the exec baton and the lease inheritance; `::advance_on_a_quiescent_empty_agent_is_a_silent_noop` and `::advance_verb_surfaces_an_unusable_workspace_loudly` cover the terminators. Warrant derivation is covered by `src/prompt/dispatch/advance/tests.rs`.
...
| **G8** | **The shipped config template declares an action the interpreter does not implement.** `template/workflow.yaml` binds `user_message: [spawn_root_agent]`, and `spawn_root_agent`/`spawn_exchange` parse and validate as `config::action::Action` — but `src/prompt/workflow_actions.rs` has no interpreter arm for either and declines anything outside its supported set with `Error::ActionUnsupported`. | Verified by grep: `src/config/action.rs:64-65` parses both; `template/workflow.yaml:3` ships `spawn_root_agent`; `workflow_actions.rs:117` is the catch-all decline. **Not currently reachable** — the live `lernie prompt` run in this pass succeeded with that template, so the `user_message` binding is never interpreted. The gap is a declaration with no implementation behind it, not a live failure. |
| **G9** | **`--stop-children` has no integration proof.** The e2e test reaches only the no-holder short-circuit; no test kills a live child through the flag. | `src/e2e/stop_idempotence.rs::stop_stop_children_flag_parses_and_is_idempotent`. The walk is unit-covered only. |
