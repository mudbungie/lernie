...
> **Shipped-state note.** The transcript is the shipped harness's sole assembly source (bl-4798 wrote it; bl-26cb re-pointed assembly at it). A settled model call seals its staging entry and commits `messages/NNN-assistant.json`; each resolved tool commits `messages/NNN-tool.json`; each step re-assembles its whole model-facing history by `readdir`+sort of `messages/` in the read-state commit's tree (below, *Origins and wire framing*), with **zero runtime content reads under `steps/`** — the pre-transcript in-memory accumulator and the `tools/<tool-id>/output.json` read path are deleted (the per-call `output.json` is still *written* as the diagnostic raw capture, never read). The one gap left is the on-ramp: the initial user message is committed directly by the root executor as `messages/NNN-user.md` (the first transcript entry), standing in for the §2.11 inbox drain until the inbox substrate lands (bl-cb44, bl-1129) — a *delivery-mechanism* stub, not an assembly one. The writer's mechanics are specified below (*The transcript writer*).
...
> **Shipped-state note.** The deposit half of the channel is shipped (bl-cb44): the executor lock (`flock` on the agent's `inbox/<agent-id>/` fd, `src/prompt/inbox/lock.rs`, acquired at the start of the shipped `lernie prompt` step loop and released on exit), the create-only deposit (`src/prompt/inbox/deposit.rs`), the `lernie message <workspace> <agent> <content>` verb, and the `message` built-in tool (`src/prompt/tool/builtin/message/`, sender from `LERNIE_CONV_BRANCH`). Deposited files **accumulate** in the inbox — the delivery half is not built: no step-boundary drain, no delivery commit (bl-1129), no startup scan (bl-d148), and the result-message return path that rides the channel is bl-4ce8. The verb's post-deposit probe is live (it releases the lease and calls the launcher), but the driver it would launch is `lernie advance` (§6), which does not yet exist, so the production launcher is a documented no-op pending that verb; the deposit is delivered by the next driver that runs against the branch once the drain lands. This section is otherwise the target contract.
