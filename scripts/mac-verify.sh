...
#
# PORTED FROM THE SIBLING COMPONENTS (thrall bl-e479, litany bl-c2b9), and kept
# deliberately close to byte-identical with them. Each component owns its own
# build tooling — they meet at the wire and nowhere else — so a copy is the
# shape rather than an oversight, and a short `diff` against a sibling is the
# only cheap defence against two copies drifting.
#
# WHY THIS EXISTS HERE. This seat's mac artifact is built NATIVELY on an Apple
# runner (bl-9380: the window links Apple frameworks, the frameworks live only
# in Apple's SDK, and the SDK agreement refuses both hosting it and running any
# part of it on non-Apple hardware — so a Linux cross build is not a missing
# flag, it is a term nobody can satisfy). Building on the right hardware removes
# the cross-compiler from the picture; it does not turn "the build succeeded"
# into evidence. A linker that quietly produced the wrong architecture, a graph
# that acquired a dependency on a dylib no stock mac carries, or a binary macOS
# would refuse to start all look identical to a green build. Every one of those
# is READABLE out of the file itself, on any platform, with no Apple tooling.
#
# WHAT IT PROVES, and the boundary is worth being exact about:
#
#   proven      the file is a 64-bit Mach-O executable for arm64; it targets the
#               macOS platform; the minimum OS it declares; every dynamic
#               library it will ask macOS for at load time; and that it carries
#               a code signature at all.
#   NOT proven  that it runs. Nothing here executes it. An artifact that passes
#               has the SHAPE of a working mac binary and has not been observed
#               to be one.
#
...
# this repository. `--self-test` feeds the reader files it MUST refuse — a
# truncated one, a non-Mach-O one, a Mach-O for the wrong architecture, one that
# is not an executable, and one whose load commands enumerate nothing — because
# a checker that has stopped checking passes everything forever. The workflow
# runs it BEFORE the build, so the negative direction is answered before the
# positive one is asked.
#
# BASH 3.2 AND `od`/`dd` ONLY. macOS ships bash 3.2 and always will, so there
# are no associative arrays and no `mapfile` here; and the reader must work on a
# box with no Rust, no python and no llvm. Both platforms this runs on are
# little-endian, which is what lets `od -tu4` read a Mach-O 64-bit
# little-endian header without byte-swapping; the magic check below is what
# catches a host where that stopped being true (a big-endian host reads
# 0xfeedfacf as 0xcffaedfe and is refused as "not a 64-bit Mach-O") rather than
...
verify() {
  ...
  # An arm64 mac refuses to start an unsigned binary outright. The signature the
  # linker applies is AD-HOC — it satisfies that rule and it is NOT
