...
fn budget_ref_write_failure_surfaces_as_a_git_error() {
    // The marker `update-ref` is git op #5 in the exhaustion path (0
    // worktree add, 1 add, 2 commit, 3 step-1 rev-parse, 4 step-2
    // rev-parse, 5 mark_exhausted update-ref). Failing it surfaces the
    // §6 exhaustion write's error arm.
    let repo = scaffold_repo_with_workflow(
        VALID_PER_REPO_PROVIDERS_YAML,
        WORKFLOW_WITH_TOKEN_BUDGET,
        Some("body"),
