...
**Descriptions-always population.** The `descriptions/**` tree the assembly above intersects against is written by a single step of the conversation-repo creation routine (§2.2), as part of the initial `main` commit: the harness copies, from the harness-root pools, every available tool's JSON schema (`<harness-root>/tools/<name>.json` → `descriptions/tools/<name>.json`, point 2) and every available skill's `SKILL.md` frontmatter (`<harness-root>/skills/<name>/SKILL.md` → `descriptions/skills/<name>.md`, the Description-always mechanism above). It is **one mechanism over two artifact kinds, not two producers.** Committing onto `main` is what lets every branch inherit the descriptors via git (§2.3) and makes context assembly intersect against the branch's own read-state tree (§2.10, §5.1) instead of re-reading mutable harness-root state — the same snapshot-not-symlink discipline as skill bodies above and as `providers.yaml` "frozen at creation" (§2.2). The harness-root pools are the single source of truth for *what this install provides*; the committed `descriptions/**` snapshot is the single source of truth for *what this conversation is pinned to see* — distinct facts that must not drift together, so the copy is a snapshot, not a mirror (PRINCIPLES, single source of truth).
