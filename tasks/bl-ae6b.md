...
## Verified gap
The shipped toolset is bash, cd, dispatch, load_skill, message, read_file (src/prompt/tool/builtin/) — every file edit rides bash (sed, heredocs, python -c). Models are heavily post-trained on structured edit/patch tool shapes; bash-only editing forfeits that, and every quoting/escaping failure costs a full model round trip.
