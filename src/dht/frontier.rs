//! A walk's state and the frontier read off it (yog bl-d00f, bl-d9c1): every
//! node heard of, which were asked, which replied — and, against the flight, the
...
impl Walk {
    ...
    /// Take one answer in. The door's reply seeds the pool, but the door is
    /// never a result; a walk node's reply joins the pool, the replied set
    /// and the outcome, and its error the outcome's errors. A reply that
