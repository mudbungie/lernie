...
fn run_happy_path_writes_branch_worktree_and_two_commits() {
    ...
    // Git sequence: 6 (control resolution from the config commit, §2.2:
    ...
    assert_eq!(runs[9].1[0], "commit");
    assert!(runs[9].1[2].contains("step 001: dispatch"));
    assert!(runs[9].1[2].contains("[ct-1-deadbeef]"));
    ...
    assert_eq!(runs[16].0, worktree);
    assert_eq!(runs[16].1, vec!["rev-parse", "HEAD"]);
}
