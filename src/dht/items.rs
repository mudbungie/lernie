...
//! in one more flight, every holder at once, each with its own deadline.
//! Nothing here trusts the commons: an item is a value only once its
//! signature verifies under the key the caller asked for.
