...
Provider APIs require paired `tool_use`/`tool_result`, so every tool returns immediately. Long-running work surfaces as a handle; `await(handle)` and `check(handle)` derive its status, and the result itself arrives as the child's result message (§2.5, §2.6) — parallelism preserved without breaking the wire protocol, content kept to its one channel.
