...
fn run_surfaces_agent_dir_create_failure() {
    // Pre-create the worktree path as a regular file. `git worktree
    // add` is stubbed, so this blocker survives into
    // `write_snapshot`'s create_dir_all, which fails because a path
    // component is a file.
    let repo = scaffold_repo(VALID_PROVIDERS_YAML, VALID_AGENTS_YAML, Some("body"));
...
fn run_surfaces_goal_write_failure() {
    // Agent dir exists but goal.md is already a directory, so the
    // file write fails.
    let repo = scaffold_repo(VALID_PROVIDERS_YAML, VALID_AGENTS_YAML, Some("body"));
    let wt = worktree_path(repo.path());
