...
> **Shipped-state note.** The v0.3 merge-back protocol this section retires is **deleted** (bl-4ce8): `src/prompt/merge/` (rebase-then-align-then-merge), the `merge=ours` `.gitattributes` and `merge.ours.driver` scaffold, `MERGE_OURS_PATHS`, and `tests/merge_ours.rs` are gone. The result-message return is shipped as its replacement: `inbox::deposit_result` (the reusable, total deposit — `epitaph:`/`terminal_ref:` frontmatter, body = terminal response iff the agent spoke; §2.11 shape) and `inbox::deposit_child_result` (its parent-derivation wrapper, a no-op for a root), wired at the terminal events that execute today in the root's `run_exchange` loop — normal completion (`final-response`) and budget exhaustion (`budget-exhausted`, §6). The **work-product transfer** (`src/prompt/dispatch/transfer.rs`) is live at delivery: a delivered message carrying `terminal_ref:` applies the fork-point→terminal diff, filtered to work products, as one commit before the delivery commit, and a diff that fails to apply is **declined loudly** at `refs/lernie/conflicted/<agent-id>`. What is *structurally wired but dormant*: the deposit call sites in `run_exchange` are no-ops today because a root has no parent inbox and the shipped subagent path (`worker.rs`) stops at the dispatch commit — children do not yet run a step loop, so no child reaches a terminal event. `deposit_result` is total and reusable so the §8 startup-scan ball (bl-d148) can deposit `died` epitaphs on a crashed child's behalf, and the child step loop lands the live child returns. The one surviving merge is compaction (`src/prompt/compactor`), a plain `--no-ff` merge with no rebase or alignment.
