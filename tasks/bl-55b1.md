...
publishes 18 — the event this seat's hold is waiting for — the hold does not
clear; it becomes permanent, printing a sentence that reads as an engine defect
rather than as a stale path in this repository's own workflow. Filed against
thrall as bl-c618 and true here verbatim.
...
- `PROTOCOL` at the root, stating this seat's vendored number; `build.rs` reads
  and validates it and writes the constant `src/channel/hello.rs` includes.
...
- `Cargo.toml` include list and the packaged-file guard gain the two build
  inputs.

The engine's half is yog bl-3e57; the other consumers are thrall bl-c618 and
yog-android's sibling. Landing this does not change the number, only where it
is written: whichever lane is vendoring 18 bumps `PROTOCOL` instead of the
`const`.