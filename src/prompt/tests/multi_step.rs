...
fn loop_runs_two_steps_when_first_completion_is_tool_use() {
    ...
    // Git op log: 9 (the start's preamble — the fork-point lineage query
    ...
    // entry add+commit) = 32. The terminal result deposit adds none: the
    ...
    assert_eq!(runs[25].1[..2], ["cat-file", "blob"]);
    ...
    assert_eq!(runs[26].1, vec!["add", "-A"]);
    assert!(runs[27].1[2].contains("transcript 003: tool"));
    ...
    assert_eq!(runs[28].1, vec!["status", "--porcelain", "--", "messages"]);
    assert_eq!(runs[29].1, vec!["rev-parse", "HEAD"]);
    assert_eq!(runs[30].1, vec!["add", "messages/004-claude-sonnet-5.json"]);
    assert!(runs[31].1[2].contains("transcript 004: claude-sonnet-5"));
