...
//! is a sliding window, not lockstep rounds (yog bl-d9c1): up to α queries in
//! the air, each with its own deadline, and the next node asked the moment any
...
impl Dht {
    ...
    /// routers answer `find_node` and never BEP 44's `get` (yog's
    /// `docs/REMOTE.md` §13.7 ruling 3, yog bl-f6e1), so a walk that asked
    /// them `q` was dark in one round; the bootstrap is a door into the
    /// keyspace, and every node it opens onto is asked `q`. One walk, not a
    /// lookup and then a second one.
    ///
    ...
    /// says is in the [`Outcome`]. So a walk whose learned
    ...
    /// door's queries hold no α slot. Measured from the deployed engine box
    /// (yog's `docs/REMOTE.md` §13.7 ruling 3), the one router that answers names a single
    ...
    /// bl-f519). Traced from the deployed engine box, three of the five
