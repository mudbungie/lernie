...
//! here is ignored, not answered: this end is never a node (yog's
//! `docs/REMOTE.md` §13.2), so it holds no routing table for anyone and
//! offers nothing to store. A reply may also say where its node saw the
//! query come from (BEP 42's `ip`), which is one node's claim and is read as
//! nothing more (`Dht::observed` votes).
